...
export default function Home() {
  ...
  const handleSimulate = async () => {
    ...
    if (refreshResult.error || !refreshResult.data?.session?.user) {
      ...
      showError('Your session expired. Please sign in again.', 'warning');
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        showError(error.message || 'Simulation failed. Please try again.', 'error');
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0F0F1E] via-[#1A1A2E] to-[#16213E]">
      ...
      {showToast && (
        <div
          className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 backdrop-blur-lg border px-4 sm:px-6 py-3 rounded-xl flex items-center gap-3 w-[calc(100%-2rem)] max-w-md transition-all ${
            toastType === 'warning'
      ...
      <div className="fixed top-4 right-4 sm:top-8 sm:right-8 z-40">
        {isCheckingAuth ? (
          <div className="backdrop-blur-lg bg-white/10 rounded-full px-4 sm:px-6 py-2 border border-white/20 flex items-center space-x-2">
            <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
            <span className="text-white text-sm hidden sm:inline">Loading...</span>
          </div>
        ) : isSyncingPayment ? (
          <div className="backdrop-blur-lg bg-emerald-500/20 rounded-full px-3 sm:px-6 py-2 border border-emerald-400/40 flex items-center space-x-2">
            <div className="w-5 h-5 border-2 border-emerald-400/50 border-t-emerald-400 rounded-full animate-spin flex-shrink-0"></div>
            <span className="text-emerald-300 text-xs sm:text-sm font-medium hidden sm:inline whitespace-nowrap">
              Syncing your new credits...
            </span>
            <span className="text-emerald-300 text-xs font-medium sm:hidden whitespace-nowrap">
              Syncing...
            </span>
          </div>
        ) : user ? (
